...
{
    $slice = array_slice($width, $index);

    echo "at line {$key}, grid widths are:\n";
    var_dump($slice);

    $gridWidths[] = max($slice);
    $index += $gridWidths;
}
$gridWidths = array_sum($gridWidths);

echo "width for each grid:\n";
var_dump($gridWidths);
echo "total width = {$gridWidths}\n";
