...
function SliderMaker($quote, $chunks){
  $quote=str_replace("\n"," ",$quote);
  	$t2=parseToCodePoints($quote);
  	$t=array();
  	foreach ($t2 as $axe)
  ...
  // $arrlen = count($wheeloffortune);
  // $rowlen = count($wheeloffortune[0]);
  $a = [];
  $index = "";
    for ($row = 0; $row < count($wheeloffortune); $row++) {
      for ($col = 0; $col < count($wheeloffortune[0]); $col++) {
        $index .= $wheeloffortune[$row][$col];
      }
      $a[$row] = $index;
      $index = "";
...
Function SplitMaker($quote,$chunks)
{
  ...
  // print_r($wheeloffortune);
	shuffle($wheeloffortune);
  // print_r($wheeloffortune);
  // echo count($wheeloffortune);
